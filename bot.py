...
class T2bot(telepot.helper.ChatHandler):
    def __init__(self, token, search, db, server, **kwargs):
        super(T2bot, self).__init__(token, **kwargs)
    ...
    def open(self, initial_msg, seed):
        if not self.db.isUser(self.chat_id):
            self.sender.sendMessage(
                'user_id: %d is not a valid user' % (self.chat_id))
    ...
    def showItems(self, items):
        keyboardItems = [[InlineKeyboardButton(text=item['title'], callback_data=str(
            index))] for index, item in enumerate(items) if item]
        self.inlineButtons = InlineKeyboardMarkup(inline_keyboard=keyboardItems)
        self.savedMsgIdentifier = self.sender.sendMessage(
            'Choose ...', reply_markup=self.inlineButtons)
    ...
    def on_chat_message(self, msg):
        ...
        if content_type == 'text':
            ...
            # command - new tv schedule
                ...
                name, weekday, time, start_date, keyword = msg['text'].split(' ')[
                    1].split('|')
                keyword = keyword.replace(',', ' ')
                for day in weekday.split(','):
                    for res in ['720', '1080']:
                        self.db.subscribeTvShow(
                            name, day, time, start_date, keyword + ' ' + res, self.chat_id)
            ...
            # schedule 되어 있는 목록을 확인한다.
                ...
                self.items = self.db.watingEpisodes(
                    self.chat_id, datetime.today().strftime('%Y%m%d'))
                if len(self.items) == 0:
            ...
            self.sender.sendMessage('You can only send text message.')
    ...
    def on_callback_query(self, msg):
        ...
        if self.mode == 'search':
            ...
            self.bot.editMessageText(id, '%s 추가 완료' %
                                     torrent['title'], reply_markup=None)
            self.addTorrent(torrent['magnet'], self.chat_id)

        elif self.mode == 'delete':
            torrent = self.items[int(data)]
            self.bot.editMessageText(id, '%s 삭제 완료' %
                                     torrent['title'], reply_markup=None)
            ...
            self.bot.editMessageText(
                id, '오늘부터 [%s]를 스케줄링 하지 않습니다.' % tvshow['title'], reply_markup=None)
            self.db.unsubscribeTvShow(
                tvshow['program_id'], datetime.today().strftime('%Y%m%d'))

        elif self.mode == 'episodes':
            episode = self.items[int(data)]
            self.bot.editMessageText(
                id, '[%s]를 스케줄에서 삭제합니다..' % episode['title'], reply_markup=None)
            self.db.deleteEpisode(
                episode['program_id'], episode['download_date'])
...
class JobMonitor(telepot.helper.Monitor):
    def __init__(self, token, search, server, db):
        super(JobMonitor, self).__init__(
            token, capture=[{'_': lambda msg: True}])
        self.server = server
        self.search = search
        self.db = db
        self.logger = logging.getLogger('torrentbot')
        ...
        self.sched.add_job(self.createDailyTvSchedule,
    ...
    def downloadTvEpisodes(self):
        ...
        for episode in schedule:
            ...
            if not len(torrents):
                ...
                self.db.increaseTvEpisodeCount(
                    episode['program_id'], episode['download_date'])
            else:
    ...
    def torrentMonitor(self):
        ...
        if not self.server.isAlive():
            self.logger.warn(
                'checking server completed list, find Server down, will reboot')
            self.server.reboot()
        else:
    ...
    def deleteOldEpisodes(self):
        ...
        for episode in episodes:
            self.db.deleteEpisode(
                episode['program_id'], episode['download_date'])
            episodesPerUser.setdefault(
                episode['chat_id'], []).append(episode['title'])
        ...
        super(ChatBox, self).__init__(token,
                                      [
                                          ...
                                          (per_application(), create_open(
                                              JobMonitor, self.search, self.server, self.db)),
                                      ])
