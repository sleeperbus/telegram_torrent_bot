...
class deluge:
    ...
    def reboot(self):
        if self.isAlive():
            ...
            os.system(
                "ps ax | grep -vw grep | grep deluged | awk '{print $1}' | xargs kill -9")
    ...
    def add(self, magnet):
        if not self.isAlive():
            self.logger.warn(
                'before adding torrent, find server down, will restart')
            self.restart()

        command = "deluge-console add " + magnet
        befTorrents = self.ongoing()
        befTorrents = [{'title': t['title'], 'id':t['id']}
                       for t in befTorrents]
        os.system(command)
        sleep(1)
    ...
    def delete(self, id):
        if not self.isAlive():
            self.logger.warn(
                'before deleting torrent, find server down, will restart')
            self.restart()
    ...
    def ongoing(self):
        if not self.isAlive():
            self.logger.warn(
                'before checking torrent, find server down, will restart')
            self.restart()
